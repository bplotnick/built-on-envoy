...
WAF inspection mode. Defaults to "REQUEST_ONLY" if not set. "RESPONSE_ONLY" is deprecated and will be removed in a future release.

| | |
|---|---|
| **Type** | `string` |
| **Required** | No |
| **Allowed values** | `REQUEST_ONLY`, `FULL`, `RESPONSE_ONLY` |
...
<a id="lookup"></a>
## DNS Gateway Lookup Configuration
...
[↑ Top](#) | [↑ DNS Gateway Lookup Configuration](#lookup) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */}




<a id="resolver"></a>
## DNS Gateway Resolver Configuration

Configuration for the dns-gateway resolver UDP listener filter, the DNS-interception half of the dns-gateway extension set.

### domains

Domain matchers, each with its own CIDR range(s) for virtual IP allocation.
...
[↑ Top](#) | [↑ DNS Gateway Resolver Configuration](#resolver)



#### domains.base_ip

Flat single-family form. Base address for virtual IP allocation, IPv4 (answered as A) or IPv6 (answered as AAAA); the address family is inferred from the value. Mutually exclusive with ipv4/ipv6.
...
Explicit IPv4 range (answers A). Combine with ipv6 to serve a domain dual-stack. Mutually exclusive with the flat base_ip/prefix_len.

| | |
|---|---|
| **Type** | [ipv4Range](#resolver-ipv4Range) |
| **Required** | No |
...
Explicit IPv6 range (answers AAAA). Combine with ipv4 to serve a domain dual-stack. Mutually exclusive with the flat base_ip/prefix_len.

| | |
|---|---|
| **Type** | [ipv6Range](#resolver-ipv6Range) |
| **Required** | No |
...
String key-value pairs exposed in Envoy filter state as io.builtonenvoy.dns_gateway.metadata.\<key\>. Shared across both families for a dual-stack matcher.
...
CIDR prefix length for the flat base_ip: 1-32 for IPv4, 1-128 for IPv6 (validated against the base_ip family at config load).
...
[↑ Top](#) | [↑ DNS Gateway Resolver Configuration](#resolver) {/* if .SubProperties */} {/* range .Properties */}



<a id="resolver-ipv4Address"></a>
### Custom type: ipv4Address

IPv4 base address (answered as an A record). {/* range .Properties */}

<a id="resolver-ipv4Range"></a>
### Custom type: ipv4Range

An IPv4 virtual-IP range (answers A queries).
...
| | |
|---|---|
| **Type** | [ipv4Address](#resolver-ipv4Address) |
| **Required** | Yes |
...
CIDR prefix length (1-32). A /24 gives 256 virtual IPs.
...
[↑ Top](#) | [↑ DNS Gateway Resolver Configuration](#resolver) {/* range .Properties */}

<a id="resolver-ipv6Address"></a>
### Custom type: ipv6Address

IPv6 base address (answered as an AAAA record). {/* range .Properties */}

<a id="resolver-ipv6Range"></a>
### Custom type: ipv6Range

An IPv6 virtual-IP range (answers AAAA queries).
...
| | |
|---|---|
| **Type** | [ipv6Address](#resolver-ipv6Address) |
| **Required** | Yes |
...
CIDR prefix length (1-128). A /64 gives 2^64 virtual IPs.
...
[↑ Top](#) | [↑ DNS Gateway Resolver Configuration](#resolver) {/* range .Properties */} {/* range .CustomTypes */} {/* if .CustomTypes */}
...
[↑ Top](#) | [↑ Token Exchange Configuration](#token-exchange) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */}




<a id="example-wasm-go"></a>
## example-wasm-go Configuration

Configuration for the example-wasm-go extension.

### header_value

Custom value for the header to add. Defaults to "example" if not set.
...
[↑ Top](#) | [↑ example-wasm-go Configuration](#example-wasm-go) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */} {/* range . */}
